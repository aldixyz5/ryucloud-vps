...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('RyuCloud_panel.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('RyuCloudPanel')
...
PANEL_NAME = os.getenv('PANEL_NAME', 'RyuCloud PANEL')
WATERMARK = os.getenv('WATERMARK', 'RyuCloud VPS Service')
WELCOME_MESSAGE = os.getenv('WELCOME_MESSAGE', 'Welcome to RyuCloud PANEL! Power Your Future!')
MAX_VPS_PER_USER = int(os.getenv('MAX_VPS_PER_USER', '3'))
DEFAULT_OS_IMAGE = os.getenv('DEFAULT_OS_IMAGE', 'ubuntu:22.04')
DOCKER_NETWORK = os.getenv('DOCKER_NETWORK', 'RyuCloud_network')
MAX_CONTAINERS = int(os.getenv('MAX_CONTAINERS', '100'))
DB_FILE = 'RyuCloud_panel.db'
BACKUP_FILE = 'RyuCloud_panel_backup.json'
...
SMTP_USER = os.getenv('SMTP_USER', 'ryuzetsu342@gmail.com')
SMTP_PASS = os.getenv('SMTP_PASS', 'Ryu_zetsu')
NOTIFICATION_EMAIL = os.getenv('NOTIFICATION_EMAIL', 'ryuzetsu342@gmail.com')
BACKUP_SCHEDULE = os.getenv('BACKUP_SCHEDULE', 'daily')
VPS_HOSTNAME_PREFIX = os.getenv('VPS_HOSTNAME_PREFIX', 'RyuCloud-')
    ...
    try:
        for vps_id, vps in db.get_all_vps().items():
            ...
            try:
                ...
                disk_usage = psutil.disk_usage(f'/var/lib/docker/volumes/RyuCloud-{vps_id}/_data').percent if os.path.exists(f'/var/lib/docker/volumes/RyuCloud-{vps_id}/_data') else 0
...
def build_custom_image(base_image=DEFAULT_OS_IMAGE, dockerfile_content=None):
    with image_build_lock:
        ...
        try:
            ...
            image_tag = f"RyuCloud/{base_image.replace(':', '-').lower()}:latest"
    ...
    if request.method == 'POST':
        try:
            ...
            container = docker_client.containers.run(
                ...
                volumes={f'RyuCloud-{vps_id}': {'bind': '/data', 'mode': 'rw'}},
...
def delete_vps(vps_id):
    ...
    try:
        ...
        container.stop()
        container.remove()
        volume = docker_client.volumes.get(f'RyuCloud-{vps["vps_id"]}')
        volume.remove()
    except:
        pass
...
def clone_vps(vps_id):
    ...
    try:
        ...
        new_image_tag = f"RyuCloud/clone-{generate_vps_id().lower()}:latest"
        ...
        new_container = docker_client.containers.run(
            ...
            security_opt=["seccomp=unconfined"],
            network=DOCKER_NETWORK,
            volumes={f'RyuCloud-{new_vps_id}': {'bind': '/data', 'mode': 'rw'}},
            restart_policy={"Name": "always"},
            ports=ports
        )
...
def admin_panel():
    ...
    with open('RyuCloud_panel.log', 'r') as f:
